...
async fn test_append_partition_data_file() {
    ...
    let mut data_file_writer_valid = data_file_writer_builder
        .clone()
        .build(DataFileWriterConfig::new(Some(Struct::from_iter([Some(
            Literal::Primitive(PrimitiveLiteral::Int(first_partition_id_value)),
        )]))))
        .await
        .unwrap();
    ...
    let mut data_file_writer_invalid = data_file_writer_builder
        .clone()
        .build(DataFileWriterConfig::new(Some(Struct::from_iter([Some(
    ...
    data_file_writer_invalid = data_file_writer_builder
        .clone()
        .build(DataFileWriterConfig::new(Some(Struct::from_iter([
