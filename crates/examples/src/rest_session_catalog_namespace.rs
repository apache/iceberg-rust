...
/// This is a simple example that demonstrates how to use [`RestSessionCatalog`] to create
/// namespaces.
///
/// Unlike [`RestCatalog`](iceberg_catalog_rest::RestCatalog), every operation
/// takes the [`SessionContext`] it should run under, so a single catalog instance
/// can serve requests on behalf of different sessions.
///
/// A running instance of the iceberg-rest catalog on port 8181 is required.
/// You can find how to run the iceberg-rest catalog with `docker compose` in
/// the official [quickstart documentation](https://iceberg.apache.org/spark-quickstart/).
#[tokio::main]
async fn main() {
    // ANCHOR: create_catalog
    // Create the REST iceberg session catalog.
    ...
    // Build the session context passed to each operation.
    ...
    let existing_namespaces = catalog.list_namespaces(&context, None).await.unwrap();
    ...
    if catalog
        .namespace_exists(&context, &namespace_ident)
        .await
        .unwrap()
    {
        println!("Namespace already exists, dropping now.",);
        catalog
            .drop_namespace(&context, &namespace_ident)
            .await
            .unwrap();
    }
    ...
    let loaded_namespace = catalog
        .get_namespace(&context, &namespace_ident)
        .await
        .unwrap();
