...
mod common {
    ...
    pub const ENV_S3_ENDPOINT: &str = "ICEBERG_TEST_S3_ENDPOINT";
    ...
    pub const ENV_S3TABLES_ENDPOINT: &str = "ICEBERG_TEST_S3TABLES_ENDPOINT";
    ...
    pub const DEFAULT_S3TABLES_PORT: u16 = 9000;

    /// Returns the S3-compatible storage endpoint (SeaweedFS).
    /// Checks ICEBERG_TEST_S3_ENDPOINT env var, otherwise returns localhost default.
    pub fn get_s3_endpoint() -> String {
        std::env::var(ENV_S3_ENDPOINT)
            .unwrap_or_else(|_| format!("http://localhost:{DEFAULT_S3_PORT}"))
    }

    /// Returns the S3Tables endpoint (SeaweedFS).
    /// S3Tables shares the same port as the S3 API on SeaweedFS.
    pub fn get_s3tables_endpoint() -> String {
        std::env::var(ENV_S3TABLES_ENDPOINT)
            .unwrap_or_else(|_| format!("http://localhost:{DEFAULT_S3TABLES_PORT}"))
    }
