...
use iceberg::{
    Catalog, CatalogBuilder, Credential, Error, ErrorKind, Namespace, NamespaceIdent, Result,
...
use crate::auth::{
    AUTH_TYPE_NONE, AUTH_TYPE_OAUTH2, AUTH_TYPE_SIGV4, AuthManager, AuthSession, AwsCredentials,
...
/// SigV4 signing region (required for SigV4 signing).
...
impl RestCatalogConfig {
        ...
        self.client.clone().unwrap_or_else(|| {
            self.default_client
                .get_or_init(|| {
                    ...
                    if self.auth_manager.is_some() || self.auth_type() == AUTH_TYPE_SIGV4 {
                        Client::builder()
    ...
    /// Explicit `rest.auth.type` (matched case-insensitively), the legacy
    /// `rest.sigv4-enabled` switch, `oauth2` when a `token`, `credential` or
    /// `oauth2-server-uri` is configured, `none` otherwise.
        ...
        match auth_type.as_str() {
            ...
            AUTH_TYPE_SIGV4 => {
                ...
                let delegate: Arc<dyn AuthManager> = match delegate_auth_type.as_deref() {
                    ...
                    Some(AUTH_TYPE_OAUTH2) | None => Arc::new(OAuth2Manager::from_config(self)?),
                    Some(other) => {
...
pub(crate) fn sigv4_signer_from_props(props: &HashMap<String, String>) -> Result<SigV4Signer> {
    ...
    let credentials = match (
        ...
        (Some(access_key_id), Some(secret_access_key)) => AwsCredentials {
            ...
            session_token: prop(REST_CATALOG_PROP_SESSION_TOKEN).map(Credential::from),
        },
        (None, None) => match (env("AWS_ACCESS_KEY_ID"), env("AWS_SECRET_ACCESS_KEY")) {
            (Some(access_key_id), Some(secret_access_key)) => AwsCredentials {
                access_key_id,
                secret_access_key: secret_access_key.into(),
                session_token: env("AWS_SESSION_TOKEN").map(Credential::from),
            },
            _ => {
                return Err(Error::new(
                    ErrorKind::DataInvalid,
...
impl Debug for RestContext {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        // The session is omitted: a custom one may print its credentials.
...
impl RestContext {
    ...
    async fn token(&self) -> Option<String> {
        self.client.token(self.session.as_ref()).await
    }

    /// Sends `request` authenticated by the catalog session.
    async fn query_catalog(&self, request: Request) -> Result<Response> {
        self.client
            .query_catalog(request, self.session.as_ref())
            .await
    }
}
...
impl RestCatalog {
    ...
    async fn context(&self) -> Result<&RestContext> {
        self.ctx
            .get_or_try_init(|| async {
                ...
                let auth_manager = self.user_config.resolve_auth_manager()?;
                ...
                let catalog_config = {
                    let init_session = auth_manager
                        .init_session(&self.user_config.client(), &self.user_config.auth_props())
                        .await?;
                    RestCatalog::load_config(&client, init_session.as_ref(), &self.user_config)
                        .await?
                };
                ...
                let session = auth_manager
                    .catalog_session(&config.client(), &config.auth_props())
                    .await?;
    ...
    async fn load_config(
        ...
        let http_response = client.query_catalog(request, session).await?;
...
impl Catalog for RestCatalog {
    async fn list_namespaces(
        ...
        loop {
            ...
            let http_response = context.query_catalog(request.build()?).await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_token_overrides_header_prop_on_the_wire() {
        // Deliberate change with the auth managers: authentication is applied
        // after extra headers (a SigV4 session must sign the final header
        // set), so a configured token now wins over `header.authorization`.
        let mut server = Server::new_async().await;
        let config_mock = create_config_mock(&mut server).await;
        let list_ns_mock = server
            .mock("GET", "/v1/namespaces")
            .match_header("authorization", "Bearer some-oauth-token")
            .with_body(r#"{"namespaces": []}"#)
            .create_async()
            .await;
    ...
    async fn test_builtin_oauth_endpoint_follows_uri_override() {
        ...
        let session_debug = format!("{:?}", context.session);
    ...
    #[tokio::test]
    async fn test_manager_uses_the_catalog_client_unless_overridden() {
        // The manager exchanges the credential over the client it is
        // handed, sharing the catalog's pool; `with_client` overrides it.
        let mut server = Server::new_async().await;
        let catalog_client_mock = server
            .mock("POST", "/v1/oauth/tokens")
            .match_header("x-from", "catalog-client")
            .with_status(200)
        ...
        let manager = OAuth2Manager::new(format!("{}/v1/oauth/tokens", server.url()))
            .with_credential(Some("client1".to_string()), "secret1".to_string());
        let session = manager
            .init_session(&tagged("catalog-client"), &HashMap::new())
            .await
            .unwrap();
        let mut req = Client::new()
        ...
        session
            ...
            .await
            .unwrap();
        catalog_client_mock.assert_async().await;

        // An explicit client wins over the catalog's.
        let own_client_mock = server
            .mock("POST", "/v1/oauth/tokens")
            .match_header("x-from", "own-client")
            .with_status(200)
            .with_body(r#"{"access_token": "tok", "token_type": "Bearer"}"#)
            .create_async()
            .await;
        ...
        session
            ...
            .await
            .unwrap();
        own_client_mock.assert_async().await;
    }
    ...
    fn test_auth_type_defaults() {
        ...
        let mixed_case = RestCatalogConfig::builder()
            ...
            .build();
        assert!(
            format!("{:?}", mixed_case.resolve_auth_manager().unwrap()).contains("OAuth2Manager")
        );
    ...
    #[test]
    fn test_legacy_sigv4_switch_wins_over_auth_type() {
        ...
        let config = RestCatalogConfig::builder()
            ...
            .build();
        assert!(
            format!("{:?}", config.resolve_auth_manager().unwrap()).contains("SigV4AuthManager")
        );
    }

    #[test]
    fn test_sigv4_signing_props_resolution() {
        // Missing region.
        let config = RestCatalogConfig::builder()
            .uri("http://localhost".to_string())
            .props(HashMap::from([(
    ...
    async fn test_sigv4_default_client_does_not_follow_redirects() {
        ...
        // Same for an injected manager without SigV4 properties.
        let config = RestCatalogConfig::builder()
            .uri(server.url())
            .auth_manager(Some(Arc::new(NoopAuthManager)))
            .build();
        let response = config
            .client()
            .get(format!("{}/v1/config", server.url()))
    ...
    async fn test_sigv4_catalog_session_rebuilds_signer_from_merged_props() {
        ...
        let session = manager
            .catalog_session(&Client::new(), &merged)
            .await
            .unwrap();
    ...
    async fn test_sigv4_default_delegate_picks_up_server_token() {
        ...
        session
            ...
            .unwrap();
        assert_eq!(
            req.headers().get("original-authorization").unwrap(),
            "Bearer srv-tok"
        );
