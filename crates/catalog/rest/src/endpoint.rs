...
/// as supporting this base set of namespace and table operations; a server that
...
mod tests {
    ...
    fn default_endpoints_cover_base_ops_but_not_optional_ones() {
        assert!(DEFAULT_ENDPOINTS.contains(&Endpoint::new(
            Method::GET,
            "/v1/{prefix}/namespaces/{namespace}/tables/{table}"
        )));
        assert!(DEFAULT_ENDPOINTS.contains(&Endpoint::new(
        ...
        // Optional endpoints (e.g. server-side scan planning) are never in the
        // default set; they must be advertised explicitly.
        assert!(!DEFAULT_ENDPOINTS.contains(&Endpoint::new(
            Method::POST,
