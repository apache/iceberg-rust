...
impl SigV4Signer {
    ...
    fn sign_at(&self, request: &mut reqwest::Request, now: DateTime<Utc>) -> Result<()> {
        ...
        // The signed `host` must match what the HTTP layer sends: reqwest/hyper
        // include an explicit non-default port in the Host header (the url crate
        // normalizes scheme-default ports away, so `port()` is None for those),
        // and the AWS SDK signs `host:port` likewise.
        let host = {
            let h = request
                .url()
                .host_str()
        ...
        // AWS SDK v2 parity (Aws4Signer default `doubleUrlEncode=true`): the
        // canonical URI is the already-percent-encoded path encoded once more,
        // keeping `/`. Unreserved-only paths are unchanged; a literal `,`
        // becomes `%2C` and an encoded `%2C` becomes `%252C`, as Java signs.
        let canonical_uri = uri_encode(request.url().path(), false);
