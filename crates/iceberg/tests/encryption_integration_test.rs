...
#[tokio::test]
async fn test_encrypted_manifest_list_loading() {
    use std::collections::HashMap;
    use std::fs;
    ...
    // ===== Setup Encryption =====
    let master_key = vec![9u8; 16];
    let kms = Arc::new(InMemoryKms::new_with_master_key(
        "manifest-list-key".to_string(),
        master_key.clone(),
    ));
    let encryption_manager = Arc::new(EncryptionManager::with_defaults(kms.clone()));
    ...
    // ===== Create Manifest List using ManifestListWriter =====
    let manifest_file = ManifestFile {
        manifest_path: format!("file://{}/metadata/manifest1.avro", table_location),
    ...
    // Write unencrypted manifest list to a temp location first
    ...
    let manifest_list_bytes = fs::read(tmp_dir.path().join("temp-manifest-list.avro")).unwrap();
    ...
    // Encrypt the manifest list content using AGS1 format
    ...
    // ===== Setup Table Metadata with Encryption Key =====
    let encrypted_key = EncryptedKey::builder()
        .key_id("manifest-list-key-1".to_string())
        .encrypted_key_metadata(key_metadata_bytes)
        .build();

    // Create table metadata with encryption keys using TableMetadataBuilder
    let schema = iceberg::spec::Schema::builder().build().unwrap();
    let partition_spec = iceberg::spec::UnboundPartitionSpec::builder().build();
    let sort_order = iceberg::spec::SortOrder::unsorted_order();

    let mut table_metadata_builder = iceberg::spec::TableMetadataBuilder::new(
        schema,
        partition_spec,
        sort_order,
    ...
    table_metadata_builder = table_metadata_builder.add_encryption_key(encrypted_key);

    let result = table_metadata_builder.build().unwrap();
    let table_metadata = result.metadata;

    // ===== Create Snapshot with Encryption Key ID =====
    let summary = Summary {
    ...
    let snapshot = iceberg::spec::Snapshot::builder()
        ...
        .with_encryption_key_id(Some("manifest-list-key-1".to_string()))
        .build();

    // ===== Load and Decrypt Manifest List =====
    let loaded_manifest_list = snapshot
    ...
    let loaded_entry = &loaded_manifest_list.entries()[0];
    assert_eq!(loaded_entry.added_files_count, Some(5));
    assert_eq!(loaded_entry.added_rows_count, Some(100));
}
