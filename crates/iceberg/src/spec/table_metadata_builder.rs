...
impl TableMetadataBuilder {
        ...
        let builder = Self {
            metadata: TableMetadata {
                ...
                default_spec: Arc::new(
                    PartitionSpec::unpartition_spec(fresh_schema.clone()).with_spec_id(-1),
    ...
    pub fn add_partition_spec(mut self, unbound_spec: UnboundPartitionSpec) -> Result<Self> {
        let new_spec_id = self.reuse_or_create_new_spec_id(&unbound_spec);
        ...
        let highest_field_id = spec.highest_field_id();
        self.metadata
            .partition_specs
    ...
    fn reuse_or_create_new_spec_id(&self, new_spec: &UnboundPartitionSpec) -> i32 {
        self.metadata
            .partition_specs
            .iter()
            .find_map(|(id, old_spec)| old_spec.is_compatible_with(new_spec).then_some(*id))
            .unwrap_or_else(|| {
...
mod tests {
    use super::*;
    use crate::spec::{
        NestedField, NullOrder, Operation, PrimitiveType, Schema, SnapshotRetention, SortDirection,
        SortField, StructType, Summary, Transform, Type, UnboundPartitionField,
    };
