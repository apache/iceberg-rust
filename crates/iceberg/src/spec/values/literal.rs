...
enum SerializableLiteral {
    ...
    AboveMax,
    BelowMin,
    Struct(Vec<Option<SerializableLiteral>>),
    List(Vec<Option<SerializableLiteral>>),
    Map(Vec<(SerializableLiteral, Option<SerializableLiteral>)>),
}

impl From<&Literal> for SerializableLiteral {
    fn from(value: &Literal) -> Self {
        match value {
            Literal::Primitive(PrimitiveLiteral::Boolean(value)) => Self::Boolean(*value),
            Literal::Primitive(PrimitiveLiteral::Int(value)) => Self::Int(*value),
            Literal::Primitive(PrimitiveLiteral::Long(value)) => Self::Long(*value),
            Literal::Primitive(PrimitiveLiteral::Float(value)) => {
                Self::Float(value.to_bits().to_be_bytes())
            }
            Literal::Primitive(PrimitiveLiteral::Double(value)) => {
                Self::Double(value.to_bits().to_be_bytes())
            }
            Literal::Primitive(PrimitiveLiteral::String(value)) => Self::String(value.clone()),
            Literal::Primitive(PrimitiveLiteral::Binary(value)) => Self::Binary(value.clone()),
            Literal::Primitive(PrimitiveLiteral::Int128(value)) => {
                Self::Int128(value.to_be_bytes())
            }
            Literal::Primitive(PrimitiveLiteral::UInt128(value)) => {
                Self::UInt128(value.to_be_bytes())
            }
            Literal::Primitive(PrimitiveLiteral::AboveMax) => Self::AboveMax,
            Literal::Primitive(PrimitiveLiteral::BelowMin) => Self::BelowMin,
            Literal::Struct(value) => Self::Struct(
                value
                    .iter()
                    .map(|value| value.map(SerializableLiteral::from))
                    .collect(),
            ),
            Literal::List(value) => Self::List(
                value
                    .iter()
                    .map(|value| value.as_ref().map(SerializableLiteral::from))
                    .collect(),
            ),
            Literal::Map(value) => Self::Map(
                value
                    .iter()
                    .map(|(key, value)| {
                        (
                            SerializableLiteral::from(key),
                            value.map(SerializableLiteral::from),
                        )
                    })
                    .collect(),
            ),
...
impl From<SerializableLiteral> for Literal {
    fn from(value: SerializableLiteral) -> Self {
        match value {
            SerializableLiteral::Boolean(value) => Self::bool(value),
            SerializableLiteral::Int(value) => Self::int(value),
            SerializableLiteral::Long(value) => Self::long(value),
            SerializableLiteral::Float(value) => {
                Self::float(f32::from_bits(u32::from_be_bytes(value)))
            }
            SerializableLiteral::Double(value) => {
                Self::double(f64::from_bits(u64::from_be_bytes(value)))
            }
            SerializableLiteral::String(value) => Self::string(value),
            SerializableLiteral::Binary(value) => Self::binary(value),
            SerializableLiteral::Int128(value) => {
                Self::Primitive(PrimitiveLiteral::Int128(i128::from_be_bytes(value)))
            }
            SerializableLiteral::UInt128(value) => {
                Self::Primitive(PrimitiveLiteral::UInt128(u128::from_be_bytes(value)))
            }
            SerializableLiteral::AboveMax => Self::Primitive(PrimitiveLiteral::AboveMax),
            SerializableLiteral::BelowMin => Self::Primitive(PrimitiveLiteral::BelowMin),
            SerializableLiteral::Struct(value) => Self::Struct(
                value
                    .into_iter()
                    .map(|value| value.map(Literal::from))
                    .collect(),
            ),
            SerializableLiteral::List(value) => Self::List(
                value
                    .into_iter()
                    .map(|value| value.map(Literal::from))
                    .collect(),
            ),
            SerializableLiteral::Map(value) => Self::Map(
                value
                    .into_iter()
                    .map(|(key, value)| (Literal::from(key), value.map(Literal::from)))
                    .collect(),
            ),
        }
    }
...
impl Serialize for Literal {
    ...
    where S: Serializer {
        SerializableLiteral::from(self).serialize(serializer)
    }
}
...
impl<'de> Deserialize<'de> for Literal {
    ...
    where D: Deserializer<'de> {
        Ok(SerializableLiteral::deserialize(deserializer)?.into())
    }
}
