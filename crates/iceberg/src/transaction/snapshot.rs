...
mod tests {
    ...
    async fn write_encrypted_manifest_roundtrips() {
        let table = make_encrypted_table();
        assert!(
        ...
        let mut producer =
            SnapshotProducer::new(&table, Uuid::new_v4(), HashMap::new(), vec![data_file()]);

        let mut writer = producer
            .new_manifest_writer(ManifestContentType::Data)
            .unwrap();
        writer
            .add_entry(
                crate::spec::ManifestEntry::builder()
        ...
        assert!(!key_metadata_bytes.is_empty());
    ...
    #[tokio::test]
    async fn write_plaintext_manifest_when_no_encryption() {
        ...
        let data_file = DataFileBuilder::default()
            ...
            .file_path("memory:///table/data/00000.parquet".to_string())
            .file_format(DataFileFormat::Parquet)
            .partition(Struct::from_iter([Some(crate::spec::Literal::long(0))]))
            .record_count(100)
            .file_size_in_bytes(4096)
            .partition_spec_id(0)
            .build()
        ...
        let mut producer = SnapshotProducer::new(&table, Uuid::new_v4(), HashMap::new(), vec![
            data_file.clone(),
        ]);
        ...
        assert!(
            ...
            "plaintext manifest must not record key metadata"
        );
        ...
        let raw = table
            ...
            .await
            .unwrap();
        Manifest::parse_avro(&raw).expect("plaintext manifest bytes must parse as Avro");
    }
}
