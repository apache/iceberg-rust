...
| `row_delta.rs` | `BaseRowDelta` | Merge-on-read commit: data + position/equality/deletion-vector delete files in one snapshot; `validate_no_conflicting_data_files/_delete_files`, `validate_data_files_exist`, always-on `validateAddedDVs` (op set `{Overwrite, Delete, Replace}`) + `validateNoConflictingFileAndPositionDeletes`; the Rust-conservative fresh-DV-only door (`validate_fresh_dvs_only` — Java merges previous deletes instead, deferred) |
