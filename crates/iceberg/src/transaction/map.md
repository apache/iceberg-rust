...
| `delete_files.rs` | `StreamingDelete` | Delete data files by path/reference; opt-in `validate_files_exist()`; `stage_only()` (WAP staging — adds the `Delete` snapshot without moving `main`) |
