...
impl CompressionCodec {
    ...
    pub(crate) fn compress(&self, bytes: Vec<u8>) -> Result<Vec<u8>> {
        match self {
            ...
            CompressionCodec::Zstd(level) => {
                ...
                let mut encoder =
                    zstd::stream::Encoder::new(writer, level.unwrap_or(ZSTD_DEFAULT_LEVEL) as i32)?;
    ...
    /// Variants without a level (`None`, `Lz4`, `Snappy`) are returned unchanged.
    pub(crate) fn with_level(self, level: Option<u8>) -> Self {
        match self {
...
mod tests {
    ...
    fn test_display() {
        ...
        assert_eq!(CompressionCodec::Zstd(Some(5)).to_string(), "Zstd(level=5)");
        assert_eq!(CompressionCodec::Gzip(None).to_string(), "Gzip");
        assert_eq!(CompressionCodec::Gzip(Some(9)).to_string(), "Gzip(level=9)");
