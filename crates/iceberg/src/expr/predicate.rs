...
pub fn visit_predicate<V: BoundPredicateVisitor>(
    visitor: &mut V,
    predicate: &BoundPredicate,
) -> Result<V::T> {
    match predicate {
        BoundPredicate::And(predicates) => visit_and(visitor, predicates.inputs()),
        BoundPredicate::Or(predicates) => visit_or(visitor, predicates.inputs()),
        BoundPredicate::Not(predicate) => visit_not(visitor, predicate.inputs()),
        BoundPredicate::AlwaysTrue => visitor.visit_always_true(),
        BoundPredicate::AlwaysFalse => visitor.visit_always_false(),
        BoundPredicate::Unary(unary) => visitor.visit_unary(unary),
        BoundPredicate::Binary(binary) => visitor.visit_binary(binary),
        BoundPredicate::Set(set) => visitor.visit_set(set),
    }
}

/// Visit an AND predicate.
fn visit_and<V: BoundPredicateVisitor>(
    visitor: &mut V,
    predicates: [&BoundPredicate; 2],
) -> Result<V::T> {
    let mut results = Vec::with_capacity(predicates.len());
    for predicate in predicates {
        let result = visit_predicate(visitor, predicate)?;
        results.push(result);
    }
    visitor.and(results)
}
...
fn visit_or<V: BoundPredicateVisitor>(
    visitor: &mut V,
    predicates: [&BoundPredicate; 2],
) -> Result<V::T> {
    let mut results = Vec::with_capacity(predicates.len());
...
fn visit_not<V: BoundPredicateVisitor>(
    visitor: &mut V,
    predicate: [&BoundPredicate; 1],
) -> Result<V::T> {
    let result = visit_predicate(visitor, predicate.first().unwrap())?;
    visitor.not(result)
}
