...
///
/// Shows all manifests from all snapshots with the referencing snapshot ID.
pub struct AllManifestsTable<'a> {
    table: &'a Table,
}

impl<'a> AllManifestsTable<'a> {
    /// Create a new AllManifests table instance.
    pub fn new(table: &'a Table) -> Self {
        Self { table }
    }

    /// Returns the iceberg schema of the all manifests table.
    ///
    /// Same as the manifests table schema plus a `reference_snapshot_id` field.
        ...
        for snapshot in self.table.metadata().snapshots() {
            ...
            for manifest in manifest_list.entries() {
                ...
                self.append_partition_summaries(
                    ...
                    &manifest.partitions.clone().unwrap_or_default(),
                    spec_struct,
                );
...
mod tests {
    ...
    #[tokio::test]
    async fn test_all_manifests_table() {
        ...
        check_record_batches(
            ...
            expect![[r#"
                ...
                Field { "reference_snapshot_id": Int64, metadata: {"PARQUET:field_id": "18"} }"#]],
            expect![[r#"
                ...
                reference_snapshot_id: PrimitiveArray<Int64>
                [
                  3055729675574597004,
                ]"#]],
            &["path", "length"],
