...
/// If a fallback ID is absent from the current top-level schema while a higher field ID remains,
/// the file is ambiguous: that gap may be a dropped physical field or an Unknown field omitted by
/// an older schema. Without a name mapping or embedded field IDs, assigning later columns would
/// risk returning them under the wrong Iceberg IDs, so reject the fallback instead. Physical
/// columns after the current schema's highest top-level ID are trailing dropped columns; leave
/// those unmapped so they cannot shadow synthesized metadata columns with the same name.
pub(super) fn add_fallback_field_ids_to_arrow_schema(
    ...
    let top_level_field_ids: HashSet<i32> = iceberg_schema
        ...
        .map(|field| field.id)
        .collect();
    let max_top_level_field_id = top_level_field_ids.iter().max().copied();
    ...
    let fields_with_fallback_ids: Vec<_> = arrow_schema
        ...
        .map(|(position, field)| {
            ...
            if !top_level_field_ids.contains(&field_id) {
                if max_top_level_field_id.is_some_and(|max_id| field_id < max_id) {
                    return Err(Error::new(
