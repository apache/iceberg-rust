...
//! | `Variants.object(metadata)` + `ShreddedObject.put` (core, plain-writer core only) | [`VariantObjectBuilder`] (`write.rs`) |
...
//! recursion, and the not-ported `ShreddedObject` shredding overlay) lives in `write.rs`'s
//! module doc.
//!
...
//! shredding (including `ShreddedObject`'s partial-shred overlay), the `variant`
//! schema-type integration + format-version gate, and file-level Java↔Rust interop beyond
//! the 1.10.0-generated byte pins in `tests.rs`.
