...
Do a dry run on each of the packages:

```
cargo publish --dry-run -p iceberg
cargo publish --dry-run -p iceberg-catalog-rest
cargo publish --dry-run -p iceberg-catalog-hms
```

Next, make sure that you're authenticated using `cargo login`, and upload the packages:

```
cargo publish -p iceberg
cargo publish -p iceberg-catalog-rest
cargo publish -p iceberg-catalog-hms
```
